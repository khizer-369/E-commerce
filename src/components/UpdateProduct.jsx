...
const UpdateProduct = ({ product }) => {
    ...
    const productUpdateHandler = async (e) => {
        ...
        try {
            const response = await axios.put("/api/admin/manage-product", formData);
    ...
    return (
        <div className="min-h-[91vh] flex flex-col justify-evenly items-center py-10 md:py-0">
            <div className="w-[95%]">
                <div className="mb-6">
                    <h2 className="text-3xl font-semibold tracking-tight">Update Product</h2>
                    <p className="text-sm text-gray-500 mt-1">Fill in the details below to update a item</p>
                </div>
                <form onSubmit={productUpdateHandler} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <button disabled={loaderStatus} type="submit" className="h-11 w-full bg-black text-white text-lg tracking-tight hover:bg-black/95 transition duration-150 flex justify-center items-center">
                            {loaderStatus ? <Loader /> : <span>Update</span>}
                        </button>
                    </div>
                </form>
            </div>
